...
repository <https://github.com/srmnitc/pybop/issues>`__. After choosing
...
1. The first step is to fork ``pybop``. A detailed tutorial on forking
   can be found
...
5. Once done, you can reinstall ``pybop`` by
...
6. If the tests work, you are almost done! If the new feature is not
   ...
   folder. ``pybop`` uses pytest for tests. `This
...
7. Add the necessary docstrings for the new functions implemented.
   ``pybop`` uses the `numpy docstring
...
9. Final step - Submit a pull request through github. Before you submit,
   ...
   feauture will be incorporated to ``pybop`` and your contributions
   will be credited.
