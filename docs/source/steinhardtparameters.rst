...
takes into account the neighbors of an atom.
...
The module can be used easily for the calculation of :math:`q_l`
parameters. Once the neighbors have been calculated using the above code
snippet, the :math:`q_l` can be calculated by, for example, :math:`q_4`
and :math:`q_6` by,
...
Assuming that the system is set up correctly as shown in the previous
section, the averaged versions can be calculated by using the
``averaged = True`` keyword.
